...
def get_type(x):
    ...
    def dispatch(T, *xs, maxlen=5):
        ...
        if max(map(len, xs)) == 0: return T
        ts = tuple(map(get_common_suptype, xs))
        if len(ts) == 1:
            t = ts[0]
            return T if t is object else T[t]
        return T[get_common_suptype(ts), ...] if len(ts) > maxlen else T[ts]
    if x is None:
        return None
    ...
    if isinstance(x, tuple):
        return dispatch(tuple, *[[a] for a in x], maxlen=5)
...
def get_suptypes(t):
    def suptypes_of_subscripted_type(t):
        ...
        return sts[:-1] + T.mro()  # sts[-1] is T[Any, Any, ...]
...
try:
    ...
    # {filename: {(lineno, funcname): {argname: [type]}}}}
    TYPE_RECS = {}
...
def annotate_def(def_node, annotations) -> bool:
    ...
    for a in all_args:
        if a.annotation is None and a.arg != 'self':
            anno = get_full_name(annos[a.arg])
            a.annotation = ast.Name(anno)
            changed = True
    if def_node.returns is None:
        anno = get_full_name(annos['return'])
...
def annotate_script(filepath, verbose=ARGS.verbose):
    ...
    starts, ends, sigs = [], [], []
    for node in defs:
        ln0, ln1 = node.lineno, node.returns.lineno
        while lines[ln1 - 1].strip()[-1:] != ':':
            ln1 += 1
        starts.append(ln0 - 1)
        ends.append(ln1)
        node.body = []  # only keep signature
        line = re.match('\s*', lines[ln0-1])[0] + ast.unparse(node)
        sigs.append(line)
        if verbose:
